const topic = require('../../../srv/redis/syntopix/commands/topic')
const spaceMan = require('../../../srv/redis/syntopix/spaceMan')
...
function registerTopicHandlers(socket, io, keysMan) {
  ...
  socket.on('move_topic_to_space', async ({ topicId, targetSpaceId, oldSpaceId }, callback) => {
    try {
      if (!topicId || !targetSpaceId) {
        throw new Error('Missing required parameters: topicId and targetSpaceId')
      }
      ...
      console.log(`✅ Moved topic ${topicId} from ${oldSpaceId} to ${targetSpaceId}`)
      ...
      console.error(`❌ Error moving topic ${topicId} to space ${targetSpaceId}:`, err)
      callback({ success: false, error: err.message })
    }
  })
  ...
  socket.on('add_space_to_topic', async ({ topicId, newSpaceId }, callback) => {
    try {
      ...
      console.error(`❌ Error adding space ${newSpaceId} to topic ${topicId}:`, err)
      callback({ success: false, error: err.message })
    }
  })
  ...
  socket.on('delete_space_from_topics', (space) => {
    ...
    topic.deleteSpace(space, keysMan).then(() => fetchAndTransformTopics(io, keysMan))
  })

  socket.on('remove_space_from_topicX', async ({ topicId, fromSpace }, callback) => {
    try {
      ...
      console.error(`❌ Error adding space ${fromSpace} to topic ${topicId}:`, err)
      callback({ success: false, error: err.message })
    }
  })
}
